...
bool NearLanding(double pos_x, double pos_y, double dist);
...
double fargmin_VectorDiff(double fx, double fy,
    double targetX, double targetY,
    double minPower, double maxPower);
void fargmin_VectorDiff2(double &power1, double &power2,
    double fx1, double fy1, double fx2, double fy2,
    double targetX, double targetY,
    double minPower1, double maxPower1,
    double minPower2, double maxPower2);
    ...
    double minPower2, double maxPower2)
{
    ...
    if (power1 >= minPower1 && power1 <= maxPower1 && power2 >= minPower2 && power2 <= maxPower2)
    {
        return;
    }
    // If the solution is out of the range, we should find the closest point on the line.
    if (power1 < minPower1 || power1 > maxPower1)
    {
        if (power1 < minPower1)
        {
            power1 = minPower1;
        ...
        power2 = fargmin_VectorDiff(fx2, fy2,
            targetX - power1 * fx1, targetY - power1 * fy1,
            minPower2, maxPower2);
        return;
    }
    if (power2 < minPower2 || power2 > maxPower2)
    {
        if (power2 < minPower2)
        {
            power2 = minPower2;
        ...
        power1 = fargmin_VectorDiff(fx1, fy1,
            targetX - power2 * fx2, targetY - power2 * fy2,
            minPower1, maxPower1);
        return;
    }
}
...
bool NearLanding(double pos_x, double pos_y, double dist)
{
    bool near = fabs(PLAT_X - pos_x) < dist && fabs(PLAT_Y - pos_y) < dist;
    printf("PLAT_X=%.2f,PLAT_Y=%.2f,pos_x=%.2f,pos_y=%.2f,near=%s\n",
        PLAT_X, PLAT_Y, pos_x, pos_y, (near ? "true" : "false"));
    return near;
}
...
bool Rotate_robust(double angle, double pos_x, double pos_y, double vx, double vy, double targetVx, double targetVy)
{
    ...
    bool prepareLanding = NearLanding(pos_x, pos_y, 30);
    ...
    if (MT_OK && !LT_OK && RT_OK)
    {
        ...
        if (angle < 44 || (angle > 46 && angle <= 225))
            Rotate(45 - angle);
        else if (angle > 225)
            Rotate(405 - angle);
        else
    ...
    if (MT_OK && LT_OK && !RT_OK)
    {
        ...
        if (angle > 316 || (angle > 135 && angle < 314))
            Rotate(315 - angle);
        else if (angle <= 135)
            Rotate(-45 - angle);
        else
...
void SingleThrusterControl(double delVx, double delVy, double angle)
{
    ...
    if (ShouldRotate)
    {
        ...
        if (goalAngle > maxGoalAngle)
            goalAngle = maxGoalAngle;
        if (goalAngle < -maxGoalAngle)
            goalAngle = -maxGoalAngle;
        ...
        if (angleDiff < -180)
            angleDiff += 360;
        if (angleDiff < -maxRotateAngle)
            angleDiff = -maxRotateAngle;
        if (angleDiff > maxRotateAngle)
            angleDiff = maxRotateAngle;
    ...
    double power = fargmin_VectorDiff(fx, fy, delVx, delVy, 0, 1);
...
void Thruster_robust(double targetVx, double targetVy, double vx, double vy, double angle, const char *name)
{
    double absTargetVel = sqrt(targetVx*targetVx+targetVy*targetVy);
    if (absTargetVel > MAX_VELOCITY)
    {
        targetVx = targetVx / absTargetVel * MAX_VELOCITY;
        targetVy = targetVy / absTargetVel * MAX_VELOCITY;
    }
...
void Safety_Override(void)
{
    ...
    **************************************************/
    ...
    bool prepareLanding = NearLanding(pos_x, pos_y, fmax(DistLimit, 150));

    printf("Robust: X=%.2f,Y=%.2f,Vx=%.2f,Vy=%.2f,Angle=%.2f %s\n",
           pos_x, pos_y, vel_x, vel_y, angle,
           (prepareLanding ? "Prepare Landing ..." : ""));
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        if (!prepareLanding)
        {
            ...
            if (i < 3 || i > 33)
                dist = fmin_Dist(dist, (pos_y - Y_MIN) / cos(alpha));
            if (i > 6 && i < 12)
                dist = fmin_Dist(dist, (X_MAX - pos_x) / sin(alpha));
            if (i > 15 && i < 21)
                dist = fmin_Dist(dist, (Y_MAX - pos_y) / cos(alpha - M_PI));
            if (i > 24 && i < 30)
                dist = fmin_Dist(dist, (pos_x - X_MIN) / sin(alpha - M_PI));
            ...
            if (dist > 0 && dist < DistLimit)
            {
                ...
                sumX -= sin(alpha) * (DistLimit - dist);
                sumY -= cos(alpha) * (DistLimit - dist);
            }
        }
        // If the lander has no obstacle in the horizontal direction
        // We should push it to the direction of the landing platform.
        if (fabs(PLAT_X - pos_x) > 10 && (((i == 9 || i == 27) && (dist < 0 || dist > DistLimit)) || prepareLanding))
        {
            sumX += (PLAT_X - pos_x) / 50;
        }
        if ((i == 18 && fabs(PLAT_X - pos_x) < 100) || prepareLanding)
        {
            sumY -= (PLAT_Y - pos_y) / 50;
        }
    }
    ...
    {
        if (Rotate_robust(angle, pos_x, pos_y, vel_x, vel_y, targetVx, targetVy))
        {
            return;
        }
